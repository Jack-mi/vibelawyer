...
对照桌面律师手工材料（`阅卷笔录.docx` + `卷宗目录.xlsx`）与当前 Agent 规格，把整条链路迭代到律师可用的颗粒度，同时**保留我方领先能力**（强制 Citation + 页码校验、证据索引总表、阅卷结论/矛盾点）。
...
对照分析原文：`output/对照分析-律师版vsAgent版.md`。
...
- 写入 `output/对照分析-律师版vsAgent版.md`
- 澄清：桌面两份文件均为律师手工版（非 Agent 产物）；本机无 Agent 运行产物可做内容级 diff
- 输出 P0/P1/P2 差距清单，并落到具体文件改动点
...
- 本机无 `data/` 卷宗 PDF，无法端到端实测；拿到数据后需重跑并做**内容级 diff**（4 次讯问、逐字质量、流水完整度、OCR 错字）
- 逐字全文显著抬高 token 成本
- v1 单进程同时只能跑一个全流程 job
- MCP `create_case` 不做 docling 预转换，首次 `start_review` 约 4 分钟/54 页
